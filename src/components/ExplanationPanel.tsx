    ...
    <AnimatePresence mode="wait">
      {!explanation ? (
        <motion.div
          ...
          className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center text-gray-500 h-40 flex items-center justify-center"
        >
          프레임을 선택하면 계산 과정이 표시됩니다
        </motion.div>
      ) : (
        <motion.div
          key={`${explanation.frameIndex}-${explanation.formula}`}
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -10 }}
          transition={{ duration: 0.2 }}
          className="bg-white border-2 border-blue-200 rounded-lg p-6 shadow-sm h-40 flex flex-col justify-center"
        >
